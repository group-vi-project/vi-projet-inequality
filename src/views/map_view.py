...
class MapView(View):
    ...
    def renderMap(self, year, position):
        try: 
            dfinq_filtered = dfinq.loc[(dfinq["Year"] == year) & (dfinq["Position"] == position)]
            gdf = gdf_base.merge(dfinq_filtered[['Region', 'Inequality']], left_on='name', right_on='Region', how='left')
