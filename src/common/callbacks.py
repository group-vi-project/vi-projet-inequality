...
def get_callbacks(app):
    ...
    # used to update the store, called whenever the
    # user changed the state of a component
    ...
    def update_store(selected_series, selected_position,
                     selected_region, selected_year, state):
    ...
    @app.callback(
        ...
        # execute this callback whenever the url changes
        Input("url", "pathname"),
        State(StoreKeys.SHARED_STATE_STORE, "data")
    )
    ...
    # retrieve the storage value in order to update
    # the the professional position selector
    @app.callback(
    ...
    # show the region or position selector,
    # depending on the value of the series selector
    @app.callback(
    ...
    @app.callback(
        ...
        Input(ComponentIds.POSITION_SELECTOR, "value"),
    )
    def render_map(year, position):
        return MapView().renderMap(year, position)
    ...
    def render_comparison(series, region, position, year):
        return ComparisonsView().renderComparison(
            series, region, position, year
        )
